...
- **Any new `config.Storage` field must be declared in `testutil/fixtures.StorageFieldSpecs`** (`testutil/fixtures/storage_fields.go`). That table is the audited list of storage fields: `AssertStorageFieldsEnumerated` reflects over `config.Storage` and fails when the struct and the table drift apart, and the round-trip tests in `internal/api`, `internal/configservice` and `internal/client` drive every mapping from it. Adding a field therefore turns those tests red until it is threaded through all of the mappings above — which is the whole point, after `username`/`endpoint_url` (#95) and `path` (#104) were each dropped in silence.
