...
1. **Identify the family.** Most BareD changes belong to a family that already has 2–3 sibling implementations:
   - database engine → `apps/api/internal/database/{mysql,postgres,redis}.go` + `factory.go`
   - storage backend → `apps/api/internal/storage/{local,s3,sftp}.go` + `factory.go`
   - notifier → `apps/api/internal/notify/{slack,email,webhook}.go` + `factory.go`
   - CLI command → `apps/api/cmd/brd/` Cobra commands
   - REST endpoint → `apps/api/internal/api/` handler + route registration, then `apps/web/src/api/client.ts` + `apps/web/src/hooks/`
   - config field → `apps/api/internal/config/config.go` + `validator.go`, then `apps/web/src/types/` + the form component
   - UI component → `apps/web/src/components/` (+ `ui/` Radix primitives), `apps/web/src/hooks/`, `apps/web/src/stores/`
...
4. **Enumerate every touch-point.** A BareD feature is rarely one file. For a pluggable subsystem it's at minimum: implementation, `factory.go` switch, `apps/api/internal/config/validator.go` allow-list, the web `types/` union, and the form. Miss one and the feature half-works. Verify each by reading it, don't assume.
...
6. **Governing guide** — the nested `AGENTS.md` for the area (`apps/api/internal/{database,storage,notify}/AGENTS.md`, `apps/api/cmd/AGENTS.md`, `apps/web/AGENTS.md`); innermost wins.
